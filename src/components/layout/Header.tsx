    ...
    <header className="relative z-50 border-b border-soft bg-[color:var(--color-shell-peach)] backdrop-blur supports-[backdrop-filter]:bg-[color:var(--color-shell-peach)] dark:border-slate-800 dark:bg-slate-950/90">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-4 py-4 md:px-6">
        <div className="flex items-center gap-3">
          <Link href="/" className="text-base font-semibold tracking-tight text-ink-primary text-ink-teal">
            María Paz Jiménez
          </Link>
        ...
        <button
          type="button"
          className="flex h-11 w-11 items-center justify-center rounded-md border border-soft text-ink-indigo-950 transition hover:bg-surface-alt focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[color:var(--color-accent-yellow)] focus-visible:ring-offset-[color:var(--color-shell-peach)] dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-900 md:hidden"
        ...
        <nav
          ...
          className={`${isMenuOpen ? "flex" : "hidden"} absolute left-0 top-full z-40 w-full border-b border-soft bg-surface px-4 py-4 text-sm font-medium text-ink-indigo-950 shadow-lg focus-within:block dark:border-slate-800 dark:bg-slate-950 md:static md:block md:w-auto md:border-none md:bg-transparent md:p-0 md:text-ink-indigo-950 md:shadow-none md:dark:text-slate-100`}
        >
